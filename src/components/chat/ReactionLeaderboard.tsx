      ...
      <div className="p-4 space-y-3">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center gap-3">
            <div className="h-6 w-6 skeleton rounded-[8px]" />
            <div className="h-8 w-8 skeleton rounded-full" />
            <div className="h-4 w-24 skeleton rounded-[8px]" />
          </div>
        ))}
      </div>
    )
  ...
  return (
    <div className="p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-sm font-medium text-white">Reaction Leaderboard</h2>
          ...
          {entries.map((entry) => (
            ...
            >
              <span className={cn(
                ...
                rankColors[entry.rank - 1] || 'text-[#56566E]'
              )}>
                #{entry.rank}
              </span>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 text-[10px] text-[#56566E]">
                  {Object.entries(entry.reaction_breakdown).map(([emoji, count]) => (
                    <span key={emoji}>{emoji} {count}</span>
                  ))}
                </div>
              </div>
              <span className="text-sm font-semibold text-accent">{entry.reaction_count}</span>
            </div>
          ))}
        </div>
      </ScrollArea>
